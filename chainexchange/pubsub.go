...
type chainPortion struct {
	// chain is a pointer to the full chain in order to avoid copying the chain across the cache
	fullChain *gpbft.ECChain
	offset    int
}
...
func (cp *chainPortion) Get() gpbft.ECChain {
	if cp.offset < 0 {
		return *cp.fullChain
	}
	return cp.fullChain.Prefix(cp.offset)
}
