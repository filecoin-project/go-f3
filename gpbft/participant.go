...
func (p *Participant) ValidateMessage(msg *GMessage) (valid ValidatedMessage, err error) {
	...
	if err != nil {
		...
		if p.alreadyValidatedMessages.Contains(msg.Vote.Instance, buf.Bytes()) {
			metrics.messageCache.Record(context.TODO(), 1, metric.WithAttributes(attrCacheHit))
			return &validatedMessage{msg: msg}, nil
		} else {
			metrics.messageCache.Record(context.TODO(), 1, metric.WithAttributes(attrCacheMiss))
		}
	}
